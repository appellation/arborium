...
pub fn build_plugins(repo_root: &Utf8Path, options: &BuildOptions) -> Result<()> {
    ...
    // Verify tree-sitter CLI and wasm-opt are available
    ...
    pool.install(|| {
        grammars.par_iter().for_each(|grammar| {
            let result = build_single_grammar_wasm(
                repo_root,
                &registry,
                grammar,
...
fn build_single_grammar_wasm(
    repo_root: &Utf8Path,
    registry: &CrateRegistry,
    grammar: &str,
    output_override: Option<&Utf8Path>,
    _version: &str,
    tree_sitter: &crate::tool::ToolPath,
    wasm_opt: &crate::tool::ToolPath,
    printer: &OutputPrinter,
) -> Result<(u64, u64, u64)> {
    printer.print_line(grammar, "Building...", false);

    let (crate_state, grammar_config) = locate_grammar(registry, grammar).ok_or_else(|| {
        report(format!(
    ...
    // The def directory contains grammar sources and queries
    let lang_dir = grammar_crate_path.parent().expect("lang directory");
    let def_dir = lang_dir.join("def");

    // Grammar source directory (contains parser.c and optionally scanner.c)
    let grammar_src_dir = def_dir.join("grammar");
    ...
    // Verify grammar source files exist
    let parser_c = grammar_src_dir.join("src/parser.c");
    if !parser_c.exists() {
        return Err(report(format!(
            "parser.c not found at {}. Run `cargo xtask gen` first.",
            parser_c
        )));
    }

    // tree-sitter build --wasm expects scanner sources in src/, but arborium
    // stores them at grammar/ root (scanner.c, tag.h, etc). Copy all .c and .h
    // files from the grammar root into src/ so the build can find them.
    let src_dir = grammar_src_dir.join("src");
    let mut copied_files: Vec<Utf8PathBuf> = Vec::new();
    if let Ok(entries) = std::fs::read_dir(&grammar_src_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if let Some(ext) = path.extension() {
                if (ext == "c" || ext == "h") && path.is_file() {
                    let file_name = path.file_name().unwrap();
                    let dest_name = file_name.to_str().unwrap();
                    let dest = src_dir.join(dest_name);
                    if !dest.exists() {
                        std::fs::copy(&path, dest.as_std_path())?;
    ...
    // Step 1: Build standard tree-sitter language .wasm using tree-sitter CLI
    // Uses wasi-sdk to compile parser.c + scanner.c into a standard
    // tree-sitter language .wasm file (just parser tables + scanner, no runtime).
    let default_c_symbol = grammar.replace('-', "_");
    let c_symbol = grammar_config
        .c_symbol
        .as_deref()
        .unwrap_or(&default_c_symbol);
    let raw_wasm_name = format!("tree-sitter-{}.wasm", c_symbol);
    let raw_wasm = plugin_output.join(&raw_wasm_name);

    fs_err::create_dir_all(&plugin_output)?;

    let mut ts_cmd = tree_sitter.command();
    ts_cmd
        .args([
            "build",
            "--wasm",
            "-o",
            raw_wasm.as_str(),
            grammar_src_dir.as_str(),
        ]);

    let result = run_streaming(ts_cmd, grammar, printer)?;

    if !result.status.success() {
        return Err(report(format!("tree-sitter build --wasm failed:\n{}", result.stderr)));
    }

    // Step 2: Optimize WASM with wasm-opt
    let dest_wasm = plugin_output.join("language.wasm");
    ...
    // Clean up temp files
    let _ = std::fs::remove_file(&raw_wasm);
    for copied_file in &copied_files {
        let _ = std::fs::remove_file(copied_file);
    }

    // Step 3: Resolve and copy query files
    let queries_dir = def_dir.join("queries");
    resolve_query_files(registry, grammar, grammar_config, &queries_dir, &plugin_output)?;

    // Step 4: Copy and update package.json
    let plugin_source = lang_dir.join("npm");
...
/// Resolve and copy query files for a grammar, handling highlights inheritance.
fn resolve_query_files(
    registry: &CrateRegistry,
    _grammar: &str,
    grammar_config: &crate::types::GrammarConfig,
    queries_dir: &Utf8Path,
    output_dir: &Utf8Path,
) -> Result<()> {
    // Resolve highlights with inheritance (prepend parent queries)
    let highlights_scm = queries_dir.join("highlights.scm");
    if highlights_scm.exists() {
        let mut combined = String::new();

        // Check for highlights_prepend (query inheritance)
        if let Some(ref queries) = grammar_config.queries {
            if let Some(ref highlights) = queries.highlights {
                if let Some(ref prepends) = highlights.prepend {
                    for prepend in prepends {
                        // Find the parent crate in the registry by crate name
                        if let Some((_, parent_state, _)) = registry
                            .configured_crates()
                            .find(|(name, _, _)| **name == prepend.crate_name)
                        {
                            let parent_dir = parent_state.crate_path.parent().expect("parent lang dir");
                            let parent_highlights = parent_dir.join("def/queries/highlights.scm");
                            if parent_highlights.exists() {
                                let parent_content = fs_err::read_to_string(&parent_highlights)?;
        ...
        let own_content = fs_err::read_to_string(&highlights_scm)?;
        combined.push_str(&own_content);
        fs_err::write(output_dir.join("highlights.scm"), &combined)?;
    } else {
        fs_err::write(output_dir.join("highlights.scm"), "")?;
    }

    // Copy injections.scm and locals.scm as-is (no inheritance)
    let injections_scm = queries_dir.join("injections.scm");
    if injections_scm.exists() {
        std::fs::copy(&injections_scm, output_dir.join("injections.scm"))?;
    } else {
        fs_err::write(output_dir.join("injections.scm"), "")?;
    }

    let locals_scm = queries_dir.join("locals.scm");
    if locals_scm.exists() {
        std::fs::copy(&locals_scm, output_dir.join("locals.scm"))?;
    } else {
        fs_err::write(output_dir.join("locals.scm"), "")?;
    }
...
/// CDN URLs are derived at runtime: `https://cdn.jsdelivr.net/npm/@arborium/{lang}@{version}/language.wasm`
...
fn build_manifest(
    ...
    for grammar in grammars {
        ...
        let local_wasm = local_root.join("language.wasm");
